	...
	if (isset($_POST['choix'])) {
		...
		if ($rayon > 0) {
			if ($_POST['choix'] === 'circonference') {
				...
				$result = 'La circonférence est: ' . calcCirconf($rayon);
			} else if ($_POST['choix'] === 'aire') {
				$result = "L'aire est:" . calcAire($rayon);
			}
		} else {
		 $erreur = "Veuillez écrire un numero superieur à 0";
		 echo $erreur;
		}
	} else {
		$erreur = 'Veuillez selecter une des options du calcul';
		echo $erreur;
	}
} else {
	$erreur = 'Veuillez écrire le rayon du cercle';
	echo $erreur;
}
